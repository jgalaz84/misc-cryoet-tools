...
def main():
    ...
    for i, file_path in enumerate(input_files):
        ...
        for img_idx, image_3d in enumerate(image_list[:processing_count]):
            ...
            for tmpl_idx, template_data in enumerate(templates):
                store_map = (tmpl_idx < args.save_ccmaps) and (img_idx < args.save_coords_map)
                tasks.append((
                    filename,
                    img_idx,
                    tmpl_idx,
    ...
    # Save coordinate maps for first save_coords_map images
    ...
    def remove_low_outliers(peaks_list, method='percentile', threshold=0.0):
        """
        Remove low-correlating outliers from a list of peak tuples (x, y, z, val).
        
        Parameters:
            peaks_list: List of peak tuples in the format (x, y, z, val)
        ...
        Returns:
            List of peak tuples with outliers removed
        """
        if not peaks_list or threshold <= 0:
            return peaks_list  # No outlier removal
            
        # Extract correlation values
        values = np.array([peak[3] for peak in peaks_list])
        
        if method == 'percentile':
            # Remove bottom N% of points
            if 0 < threshold < 1:
                cutoff = np.percentile(values, threshold * 100)
                logging.info(f"Removing {threshold*100:.1f}% lowest correlating points (CC < {cutoff:.4f})")
            else:
                return peaks_list  # Invalid percentile
            ...
            if std_val > 0:
                ...
                logging.info(f"Removing points with z-score < -{threshold:.1f} (CC < {cutoff:.4f})")
            else:
                return peaks_list  # Can't compute z-score with std=0
            ...
            if iqr > 0:
                ...
                logging.info(f"Removing points below Q1-{threshold:.1f}*IQR (CC < {cutoff:.4f})")
            else:
                return peaks_list  # Can't use IQR when it's 0
        else:
            return peaks_list  # Unknown method
            
        # Filter peaks
        filtered_peaks = [peak for peak in peaks_list if peak[3] >= cutoff]
        removed_count = len(peaks_list) - len(filtered_peaks)
        
        if removed_count > 0:
            logging.info(f"Removed {removed_count} outliers ({removed_count/len(peaks_list)*100:.1f}% of total)")
            
        return filtered_peaks

    # Apply outlier removal if requested
    if args.drop_outliers > 0:
        logging.info(f"Applying outlier removal with {args.outlier_method} method and threshold {args.drop_outliers}")
        for key in final_peaks_by_img:
            final_peaks_by_img[key] = remove_low_outliers(
                final_peaks_by_img[key], 
                method=args.outlier_method, 
                threshold=args.drop_outliers
            )
            
        # Update aggregated data after outlier removal
        aggregated_ccs = defaultdict(list)
        aggregated_avgs = defaultdict(list)
        
        for (filename, image_idx), final_peaks in final_peaks_by_img.items():
            # Rebuild the aggregated collections
            if args.npeaks and args.npeaks > 0:
                if len(final_peaks) >= args.npeaks:
                    aggregated_ccs[filename].extend(final_peaks[:args.npeaks])
                else:
