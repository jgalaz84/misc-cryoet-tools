...
def run_bootstrap_analysis(image_stack1, image_stack2, n_rounds, sample_pct, output_dir, threads, options):
    ...
    for i in range(n_rounds):
        ...
        print(f"Round {i+1}: Sampled {len(subset1)} volumes from stack1 and {len(subset2)} volumes from stack2.")
    ...
    summary_file = os.path.join(output_dir, "bootstrap_summary_results.txt")
    ...
    print(f"Saved bootstrap summary to {summary_file}")
    return results
...
def main():
    ...
    parser.add_argument('--sample_pct', type=float, default=0.5, help="Fraction or percentage of images to sample in each bootstrap iteration.")
    ...
    # Convert sample_pct to fraction if given as a percentage (i.e. > 1)
    sample_pct = args.sample_pct
    if sample_pct > 1:
        print(f"Converting sample_pct from percentage {sample_pct} to fraction.")
    ...
    results = run_bootstrap_analysis(image_stack1, image_stack2, args.n_rounds, sample_pct, output_dir, args.threads, options)
